    ...
    reader.onload = async (ev) => {
      ...
      try {
        ...
        const res = await fetch('/api/ai', {
          ...
          body: JSON.stringify({
            type: 'checkup_scan',
            caseData: { imageBase64: b64data, imageMime: mime }
          })
        })
        ...
        // 기존 입력값과 병합 (새 값으로 덮어쓰되, null은 유지)
        setNewItems(p => ({ ...p, ...extracted }))
        if (parsed.checkupDate && !newDate) setNewDate(parsed.checkupDate)
        const count = Object.keys(extracted).length
        const result = count > 0
          ? ('사진 ' + scanImgs.length + ': ' + count + '개 항목 판독 완료')
          : ('사진 ' + scanImgs.length + ': 수치를 찾지 못했습니다')
  ...
  const addModalJsx = showAdd ? (
    <Sheet title="건강검진 기록 추가" onClose={() => { setShowAdd(false); setScanImg(null); setScanError(null); setScanImgs([]); setScanResults([]) }}>
      <div style={{ marginBottom:16, background:'#eff6ff', borderRadius:10, padding:'12px 14px', border:'1px solid #bfdbfe' }}>
        ...
        {scanning && <div style={{ marginTop:8, fontSize:12, color:'#2563eb' }}>AI 판독 중... (10~20초 소요)</div>}
        {scanResults.length > 0 && (
          <div style={{ marginTop:8 }}>
            {scanResults.map((r, i) => (
              <div key={i} style={{ fontSize:11, color: r.includes('완료') ? '#0F6E56' : '#dc2626', background: r.includes('완료') ? '#f0faf5' : '#fee2e2', borderRadius:6, padding:'5px 9px', marginBottom:4, lineHeight:1.5 }}>
                {r}
              </div>
            ))}
            <div style={{ fontSize:11, color:'#6b7280', marginTop:4 }}>여러 장 판독 시 수치가 누적 반영됩니다. 수동으로 수정 가능합니다.</div>
          </div>
        )}
        {scanImgs.length > 0 && (
          <div style={{ display:'flex', gap:6, marginTop:8, flexWrap:'wrap' }}>
            {scanImgs.map((img, i) => (
              <img key={i} src={img} alt={'검진결과지' + (i+1)} style={{ width:72, height:72, objectFit:'cover', borderRadius:7, border:'1px solid #bfdbfe', opacity: scanning && i === scanImgs.length-1 ? 0.5 : 1 }} />
            ))}
          </div>
        )}
      </div>
      ...
      {checkups.length > 0 && (() => {
        ...
        return (
          <div style={{ background:'#fef2f2', borderRadius:10, padding:'10px 13px', marginBottom:12, border:'1px solid #fecaca' }}>
            <div style={{ fontSize:12, fontWeight:700, color:'#991b1b', marginBottom:6 }}>[!] 최근 검진 이상 항목 ({latest.date})</div>
