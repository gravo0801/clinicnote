...
function Sparkline({ values, color = '#C2410C', width = 100, height = 36 }) {
...
function AiCheckupAnalysis({ abnormalItems, findingItems, memberInfo }) {
  ...
  const RISK_STYLE = {
    ...
    '정상': { bg:'#FEF7F0', color:'#C2410C', border:'#FB923C' },
  }
  const riskSt = result ? (RISK_STYLE[result.riskLevel] || RISK_STYLE['경계']) : null

  return (
    <div style={{ marginTop:14, borderTop:'1px solid #F3EFE7', paddingTop:12 }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        <div>
          <div style={{ fontSize:13, fontWeight:700, color:'#1C1917' }}>AI 검진 결과 분석</div>
          <div style={{ fontSize:11, color:'#9ca3af', marginTop:2 }}>이상 항목 기반 의사 관점 종합 분석</div>
        </div>
        <button onClick={analyze} disabled={loading}
          style={{ padding:'7px 16px', borderRadius:8, border:'none', background:loading?'#d1d5db':'#C2410C', color:'#fff', fontSize:12, fontWeight:700, cursor:loading?'not-allowed':'pointer', flexShrink:0 }}>
          {loading ? '분석 중...' : 'AI 분석'}
        </button>
      </div>
      ...
      {result && open && (
        <div style={{ marginTop:12 }}>
          ...
          {/* 임상 소견 */}
          {result.impression && (
            <div style={{ background:'#FAF7F1', borderRadius:9, padding:'11px 14px', marginBottom:10, border:'1px solid #F3EFE7' }}>
              <div style={{ fontSize:11, fontWeight:700, color:'#9ca3af', marginBottom:5 }}>임상적 의의 / 예상 진단</div>
              <div style={{ fontSize:13, color:'#1C1917', lineHeight:1.8 }}>{result.impression}</div>
            </div>
          )}
          {/* 환자 설명 */}
          {result.explanation && (
          ...
          {result.lifestyle && result.lifestyle.length > 0 && (
            <div style={{ background:'#FEF7F0', borderRadius:9, padding:'11px 14px', marginBottom:10, border:'1px solid #FB923C' }}>
              <div style={{ fontSize:11, fontWeight:700, color:'#C2410C', marginBottom:5 }}>생활습관 교정</div>
              {result.lifestyle.map((item, i) => (
                <div key={i} style={{ fontSize:13, color:'#1C1917', lineHeight:1.7, paddingLeft:12, position:'relative' }}>
                  <span style={{ position:'absolute', left:0, color:'#C2410C' }}>-</span>{item}
                </div>
              ))}
            </div>
          )}
          ...
          {/* 진료 메모 */}
          {result.doctorNote && (
            <div style={{ background:'#FAF7F1', borderRadius:9, padding:'10px 13px', border:'1px solid #e5e7eb' }}>
...
export default function HealthCheckup({ memberId, memberGender }) {
  ...
  const iStyle = { width:'100%', padding:'7px 9px', borderRadius:7, border:'1px solid #e5e7eb', fontSize:13, outline:'none', boxSizing:'border-box', fontFamily:'inherit', background:'#fff', color:'#1C1917' }
  ...
  const addModalJsx = (showAdd || editCheckup) ? (
    <Sheet title={editCheckup ? '건강검진 기록 수정' : '건강검진 기록 추가'} onClose={closeForm}>
      ...
      <div style={{ marginBottom:16, background:'#eff6ff', borderRadius:10, padding:'12px 14px', border:'1px solid #bfdbfe' }}>
        ...
        {scanResults.map((r,i) => <div key={i} style={{ fontSize:11, color:r.includes('입력')?'#C2410C':'#dc2626', background:r.includes('입력')?'#FEF7F0':'#fee2e2', borderRadius:6, padding:'4px 8px', marginTop:5 }}>{r}</div>)}
        ...
        return (
          <div key={cat.key} style={{ marginBottom:8, border:'1px solid #e5e7eb', borderRadius:10, overflow:'hidden' }}>
            <button onClick={() => setOpenCats(p => ({...p,[cat.key]:!p[cat.key]}))}
              style={{ width:'100%', display:'flex', justifyContent:'space-between', alignItems:'center', padding:'9px 12px', background:filledCount>0?'#FEF7F0':'#f9fafb', border:'none', cursor:'pointer', textAlign:'left' }}>
              <span style={{ fontSize:13, fontWeight:700, color:filledCount>0?'#C2410C':'#374151' }}>{cat.label}</span>
              <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                {filledCount > 0 && <span style={{ fontSize:10, color:'#C2410C', background:'#FFEDD5', borderRadius:10, padding:'1px 8px', fontWeight:700 }}>{filledCount}개 입력</span>}
            ...
            {openCats[cat.key] && (
              <div style={{ padding:'10px 12px 12px', display:'grid', gridTemplateColumns:cat.items.some(i=>i.type==='text')?'1fr':'1fr 1fr', gap:8 }}>
                {cat.items.map(item => (
                  <div key={item.key}>
                    <label style={{ ...lblStyle, color:newItems[item.key]?'#C2410C':'#6b7280' }}>
                    ...
                    {item.type === 'text'
                      ...
                      : <input type="number" step="any" value={newItems[item.key]||''} onChange={e => setNewItems(p => ({...p,[item.key]:e.target.value}))}
                          placeholder="-" style={{ ...iStyle, textAlign:'center', background:newItems[item.key]?'#FEF7F0':'#fff', borderColor:newItems[item.key]?'#FB923C':'#e5e7eb' }} />
                    }
                  </div>
                ))}
              </div>
      ...
      <button onClick={saveCheckup} disabled={saving}
        style={{ width:'100%', padding:'11px', background:'#C2410C', color:'#fff', border:'none', borderRadius:8, fontSize:14, fontWeight:700, cursor:saving?'not-allowed':'pointer', opacity:saving?0.7:1 }}>
  ...
  const trendModalJsx = showTrend ? (
    <div style={{ position:'fixed', inset:0, zIndex:8000, background:'rgba(0,0,0,0.4)', display:'flex', flexDirection:'column', overflow:'hidden' }}>
      <div style={{ background:'#fff', display:'flex', flexDirection:'column', height:'100%', maxWidth:1100, width:'100%', margin:'0 auto', borderRadius:'0 0 0 0' }}>
        {/* 헤더 */}
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'16px 20px', borderBottom:'1px solid #F3EFE7', flexShrink:0 }}>
          <div>
        ...
        {/* 카테고리 탭 */}
        <div style={{ display:'flex', gap:5, padding:'10px 20px', borderBottom:'1px solid #F3EFE7', flexShrink:0, overflowX:'auto' }}>
          <button onClick={() => setTrendCatFilter('all')}
            style={{ padding:'5px 12px', borderRadius:16, border:trendCatFilter==='all'?'none':'1px solid #e5e7eb', background:trendCatFilter==='all'?'#C2410C':'#fff', color:trendCatFilter==='all'?'#fff':'#6b7280', fontSize:12, cursor:'pointer', whiteSpace:'nowrap', fontWeight:trendCatFilter==='all'?700:400 }}>
            전체
          </button>
          {CHECKUP_CATEGORIES.filter(cat => cat.items.some(item => trendData[item.key]?.length >= 1)).map(cat => (
            <button key={cat.key} onClick={() => setTrendCatFilter(cat.key)}
              style={{ padding:'5px 12px', borderRadius:16, border:trendCatFilter===cat.key?'none':'1px solid #e5e7eb', background:trendCatFilter===cat.key?'#C2410C':'#fff', color:trendCatFilter===cat.key?'#fff':'#6b7280', fontSize:12, cursor:'pointer', whiteSpace:'nowrap', fontWeight:trendCatFilter===cat.key?700:400 }}>
              {cat.label}
            </button>
          ))}
        </div>
          ...
          {checkups.length === 0 ? (
            ...
            return catsToShow.map(cat => {
              ...
              return (
                <div key={cat.key}>
                  {/* 카테고리 헤더 */}
                  <div style={{ padding:'12px 20px 6px', background:'#F9F6F1', borderTop:'1px solid #e5e7eb', borderBottom:'1px solid #e5e7eb', fontSize:12, fontWeight:700, color:'#374151', position:'sticky', top:0, zIndex:1 }}>
                    {cat.label}
                  </div>
                    ...
                    <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12, minWidth: 200 + dates.length * 90 }}>
                      ...
                      <tbody>
                        {hasData.map((item, rowIdx) => {
                          ...
                          return (
                            <tr key={item.key} style={{ background: rowIdx%2===0 ? '#fff' : '#fafafa', borderBottom:'1px solid #F3EFE7' }}>
                              {/* 항목명 - sticky */}
                              ...
                              <td style={{ textAlign:'center', padding:'4px 12px' }}>
                                {isText ? (
                                  ...
                                  <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:2 }}>
                                    <Sparkline values={numValues} color={ws ? (STATUS_COLORS[ws]||'#C2410C') : '#C2410C'} width={70} height={28} />
        ...
        {/* 범례 */}
        <div style={{ padding:'10px 20px', borderTop:'1px solid #F3EFE7', display:'flex', gap:16, flexWrap:'wrap', flexShrink:0, background:'#FAF7F1' }}>
    ...
    <div>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
        ...
        <div style={{ display:'flex', gap:6 }}>
          ...
          <button onClick={() => setShowAdd(true)} style={{ padding:'6px 14px', borderRadius:7, background:'#C2410C', color:'#fff', border:'none', fontSize:12, fontWeight:700, cursor:'pointer' }}>+ 검진 추가</button>
        </div>
      </div>
      ...
      {checkups.length === 0
        ? <div style={{ textAlign:'center', padding:'40px 0', color:'#9ca3af', fontSize:13 }}>
            ...
            <button onClick={() => setShowAdd(true)} style={{ padding:'8px 20px', borderRadius:20, background:'#C2410C', color:'#fff', border:'none', fontSize:13, fontWeight:700, cursor:'pointer' }}>첫 검진 기록 추가</button>
          </div>
        : checkups.map(chk => {
            ...
            return (
              <div key={chk.id} style={{ background:'#fff', borderRadius:12, marginBottom:12, border: (abnormal.length>0||findingAbnormal.length>0) ? '1px solid #fde68a' : '1px solid #F3EFE7', overflow:'hidden' }}>
                {/* 카드 헤더 */}
                <div style={{ padding:'14px 16px', cursor:'pointer', display:'flex', justifyContent:'space-between', alignItems:'flex-start' }} onClick={() => setExpandedCheckup(isExpanded ? null : chk.id)}>
                  <div>
                    <div style={{ fontSize:14, fontWeight:700, color:'#1C1917', marginBottom:4 }}>{chk.date} 검진</div>
                ...
                {isExpanded && (
                  <div style={{ borderTop:'1px solid #F3EFE7', padding:'16px' }}>
                    {CHECKUP_CATEGORIES.map(cat => {
                      const catItems = cat.items.filter(item => chk.items?.[item.key] != null && chk.items[item.key] !== '')
                      if (catItems.length === 0) return null
                      return (
                        <div key={cat.key} style={{ marginBottom:16 }}>
                          <div style={{ fontSize:11, fontWeight:700, color:'#9ca3af', marginBottom:8, paddingBottom:4, borderBottom:'1px solid #F3EFE7', textTransform:'uppercase', letterSpacing:'0.5px' }}>{cat.label}</div>
                          {true && (
                              <div>
                                {catItems.filter(item => item.type === 'text').length > 0 && (
                                  <div style={{ display:'flex', flexDirection:'column', gap:6, marginBottom:8 }}>
                                    {catItems.filter(item => item.type === 'text').map(item => {
                                      ...
                                      return (
                                        <div key={item.key} style={{ background:isAbnormal?'#fee2e2':'#FAF7F1', borderRadius:7, padding:'8px 11px', border:isAbnormal?'1px solid #fca5a5':'none' }}>
                                          <div style={{ fontSize:11, color:'#9ca3af', marginBottom:3 }}>{item.label}</div>
                                          <div style={{ fontSize:13, color:isAbnormal?'#991b1b':'#1C1917', lineHeight:1.6 }}>{chk.items[item.key]}</div>
                                        </div>
                                      )
                                    })}
                                  </div>
                                ...
                                {catItems.filter(item => item.type !== 'text').map(item => {
                                  ...
                                  return (
                                    <div key={item.key} style={{ background:ws?(STATUS_COLORS[ws]||'#6b7280')+'12':'#FAF7F1', borderRadius:7, padding:'7px 9px', border:ws?'1px solid '+(STATUS_COLORS[ws]||'#6b7280')+'30':'none' }}>
                    ...
                    {(chk.imagingFiles||[]).length > 0 && (
                      <div style={{ marginBottom:14 }}>
                        <div style={{ fontSize:11, fontWeight:700, color:'#9ca3af', marginBottom:8, paddingBottom:4, borderBottom:'1px solid #F3EFE7', textTransform:'uppercase', letterSpacing:'0.5px' }}>영상검사 이미지</div>
