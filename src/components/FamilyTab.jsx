...
function MemberInitial({ name, size = 40 }) {
  ...
  const colors = ['#00C07F','#2563eb','#7c3aed','#db2777','#d97706','#059669']
  const bg = colors[name?.charCodeAt(0) % colors.length] || '#00C07F'
  return (
  ...
  return (
    <div style={{ paddingBottom: 16 }}>
      ...
      <div style={{ marginBottom: 12 }}>
        ...
        <label style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 12px', background: '#EDFFF8', color: '#00C07F', border: '1px dashed #5DE8BC', borderRadius: 7, fontSize: 12, cursor: 'pointer', fontWeight: 600 }}>
    ...
    <div style={{ display: 'flex', gap: 0, marginBottom: 4 }}>
      ...
      <div style={{ flex: 1, marginBottom: 12 }}>
        <div onClick={() => setOpen(p => !p)}
          style={{ background: '#fff', borderRadius: 12, padding: '12px 14px', cursor: 'pointer', border: fu === 'overdue' ? '1px solid #fca5a5' : fu === 'soon' ? '1px solid #fcd34d' : '1px solid #F0F4F8', boxShadow: open ? '0 2px 8px rgba(0,0,0,0.06)' : 'none' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: '#0D1117', marginBottom: 4 }}>{r.title}</div>
        ...
        {open && (
          <div style={{ background: '#fff', borderRadius: '0 0 12px 12px', padding: '12px 14px', borderTop: '1px solid #F0F4F8', border: '1px solid #F0F4F8', borderTopWidth: 0 }}>
            {r.note && (
              <div style={{ fontSize: 13, color: '#374151', lineHeight: 1.8, whiteSpace: 'pre-wrap', marginBottom: imgCount > 0 || fileCount > 0 ? 12 : 0, background: '#FAFBFC', borderRadius: 7, padding: '10px 12px', border: '1px solid #F0F4F8' }}>
                {r.note}
              </div>
            )}
            {imgCount > 0 && (
            ...
            <div style={{ display: 'flex', gap: 6, marginTop: 12, paddingTop: 10, borderTop: '1px solid #F0F4F8' }}>
...
export default function FamilyTab() {
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 8000, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', overflowY: 'auto', padding: '16px 12px' }}>
      <div style={{ background: '#fff', borderRadius: 14, width: '100%', maxWidth: 680, padding: '24px 28px', boxShadow: '0 20px 60px rgba(0,0,0,0.2)', marginTop: 20, marginBottom: 20 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, paddingBottom: 14, borderBottom: '1px solid #F0F4F8' }}>
          <div>
            <div style={{ fontSize: 16, fontWeight: 700, color: '#0D1117' }}>{editRecord ? '기록 수정' : '이상 내용 기록'}</div>
    ...
    <>
      {addMember && (
        <Sheet title="가족 추가" onClose={() => setAddMember(false)}>
          ...
          <button onClick={saveMember} style={{ width: '100%', padding: '12px', background: '#00C07F', color: '#fff', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>추가하기</button>
        </Sheet>
      )}
      {delConfirm && (
  ...
  if (isMobile) {
    return (
      <div style={{ paddingBottom: 80 }}>
            ...
            {members.map(m => {
              ...
              return (
                <button key={m.id} onClick={() => setSelId(m.id)}
                  style={{ padding: '7px 16px', borderRadius: 20, border: active ? 'none' : '1px solid #e5e7eb', background: active ? '#00C07F' : '#fff', color: active ? '#fff' : '#374151', fontSize: 13, fontWeight: active ? 700 : 400, cursor: 'pointer', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: 5 }}>
                  {m.name}
        ...
        {sel && (
          <div style={{ margin: '0 16px 14px', borderRadius: 16, padding: '16px 20px', background: '#00C07F', color: '#fff' }}>
        ...
        <div style={{ padding: '0 16px' }}>
          <div style={{ display: 'flex', gap: 4, background: '#F0F4F8', borderRadius: 10, padding: 4, marginBottom: 14 }}>
            {[['tracking','트래킹'],['checkup','건강검진']].map(([k,l]) => (
              <button key={k} onClick={() => setMemberTab(k)} style={{ flex: 1, padding: '7px 0', borderRadius: 7, border: 'none', background: memberTab === k ? '#fff' : 'transparent', color: memberTab === k ? '#00C07F' : '#9ca3af', fontSize: 13, fontWeight: memberTab === k ? 700 : 400, cursor: 'pointer' }}>{l}</button>
            ))}
          </div>
          {memberTab === 'checkup' ? (
                ...
                <div style={{ display: 'flex', gap: 5 }}>
                  {[['all','전체'],['followup','추적'],['ongoing','진행'],['resolved','완료']].map(([v,l]) => (
                    <button key={v} onClick={() => setFilterStatus(v)}
                      style={{ padding: '4px 10px', borderRadius: 12, border: filterStatus === v ? 'none' : '1px solid #e5e7eb', background: filterStatus === v ? '#00C07F' : '#fff', color: filterStatus === v ? '#fff' : '#6b7280', fontSize: 11, cursor: 'pointer', fontWeight: filterStatus === v ? 700 : 400 }}>{l}</button>
                  ))}
                </div>
                <button onClick={() => setAddRecord(true)} style={{ padding: '6px 14px', borderRadius: 20, background: '#00C07F', color: '#fff', border: 'none', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>+ 기록</button>
              </div>
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      <div style={{ width: 260, background: '#fff', borderRight: '1px solid #EDF0F4', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '18px 14px 12px', borderBottom: '1px solid #F0F4F8', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontSize: 13, fontWeight: 700, color: '#374151' }}>가족 구성원</span>
          <button onClick={() => setAddMember(true)} style={{ background: '#00C07F', color: '#fff', border: 'none', borderRadius: 20, padding: '4px 12px', fontSize: 12, cursor: 'pointer', fontWeight: 600 }}>+ 추가</button>
        </div>
          ...
          {members.map(m => {
            ...
            return (
              <button key={m.id} onClick={() => { setSelId(m.id); setMemberTab('tracking') }}
                style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '12px', borderRadius: 12, border: 'none', background: active ? '#EDFFF8' : 'transparent', cursor: 'pointer', marginBottom: 3, textAlign: 'left', borderRadius: 12 }}>
                <MemberInitial name={m.name} size={40} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 14, fontWeight: active ? 700 : 500, color: active ? '#00C07F' : '#0D1117' }}>{m.name}</div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', background: '#F4F6F9' }}>
        {!sel
          ...
          : <div style={{ maxWidth: 820, padding: '28px 32px' }}>
              <div style={{ borderRadius: 18, padding: '22px 28px', background: '#00C07F', color: '#fff', marginBottom: 22 }}>
              ...
              <div style={{ display: 'flex', gap: 4, background: '#F0F4F8', borderRadius: 10, padding: 4, marginBottom: 20 }}>
                {[['tracking','트래킹'],['checkup','건강검진']].map(([k,l]) => (
                  <button key={k} onClick={() => setMemberTab(k)} style={{ flex: 1, padding: '8px 0', borderRadius: 7, border: 'none', background: memberTab === k ? '#fff' : 'transparent', color: memberTab === k ? '#00C07F' : '#9ca3af', fontSize: 13, fontWeight: memberTab === k ? 700 : 400, cursor: 'pointer', boxShadow: memberTab === k ? '0 1px 3px rgba(0,0,0,0.07)' : 'none' }}>{l}</button>
                ))}
              </div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                    <div style={{ display: 'flex', gap: 6 }}>
                      {[['all','전체'],['followup','추적필요'],['ongoing','진행중'],['resolved','완료']].map(([v,l]) => (
                        <button key={v} onClick={() => setFilterStatus(v)}
                          style={{ padding: '5px 12px', borderRadius: 16, border: filterStatus === v ? 'none' : '1px solid #e5e7eb', background: filterStatus === v ? '#00C07F' : '#fff', color: filterStatus === v ? '#fff' : '#6b7280', fontSize: 12, cursor: 'pointer', fontWeight: filterStatus === v ? 700 : 400 }}>{l} {v === 'all' ? '(' + recs.length + ')' : '(' + recs.filter(r => r.status === v).length + ')'}</button>
                      ))}
                    </div>
                    <button onClick={() => setAddRecord(true)} style={{ padding: '8px 18px', borderRadius: 20, background: '#00C07F', color: '#fff', border: 'none', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>+ 기록 추가</button>
                  </div>
                  {filteredRecs.length === 0
                    ? <div style={{ textAlign: 'center', padding: '60px 0', color: '#9ca3af' }}>
                        <div style={{ fontSize: 14, marginBottom: 12 }}>기록이 없습니다</div>
                        <button onClick={() => setAddRecord(true)} style={{ padding: '9px 22px', borderRadius: 20, background: '#00C07F', color: '#fff', border: 'none', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>첫 기록 추가하기</button>
                      </div>
                    : <div>{filteredRecs.map(r => <IssueCard key={r.id} r={r} onEdit={setEditRecord} onDelete={deleteRecord} />)}</div>
                  }
                </>
