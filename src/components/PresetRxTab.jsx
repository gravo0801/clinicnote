...
const iStyle = { width:'100%', padding:'8px 10px', borderRadius:7, border:'1px solid #e5e7eb', fontSize:13, outline:'none', boxSizing:'border-box', fontFamily:'inherit', background:'#fff', color:'#1C1917' }
    ...
    <div ref={wrapRef} style={{ position:'relative' }}>
      ...
      {open && hits.length > 0 && (
        <div style={{
          ...
          zIndex:9999, background:'#fff', border:'1px solid #FED7AA', borderRadius:6,
          ...
          {hits.map(n => (
            <div key={n} onMouseDown={e => { e.preventDefault(); onChange(n); setOpen(false) }}
              style={{ padding:'8px 11px', fontSize:12, cursor:'pointer', borderBottom:'1px solid #f0f0f0', color:'#1C1917', whiteSpace:'nowrap' }}
              onMouseEnter={e => e.currentTarget.style.background='#FEF7F0'} onMouseLeave={e => e.currentTarget.style.background='#fff'}>
              💊 {n}
            </div>
          ))}
        </div>
    ...
    <div>
        ...
        <div style={{ display:'flex', flexWrap:'wrap', gap:6 }}>
          {CATEGORIES.filter(c => c !== '전체').map(c => (
            <button key={c} onClick={() => setCategory(c)}
              style={{ padding:'5px 12px', borderRadius:20, border: category===c?'none':'1px solid #e5e7eb', background: category===c?'#C2410C':'#fff', color: category===c?'#fff':'#6b7280', fontSize:12, cursor:'pointer', fontWeight: category===c?600:400 }}>
              {c}
            </button>
          ))}
        </div>
      ...
      <div style={{ marginBottom:14 }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8 }}>
          ...
          <button onClick={addDrug} style={{ background:'#FEF7F0', color:'#C2410C', border:'none', borderRadius:6, padding:'3px 10px', fontSize:11, fontWeight:700, cursor:'pointer' }}>+ 추가</button>
        </div>
        {drugs.map((d, i) => (
          <div key={i} style={{ background:'#FAF7F1', borderRadius:9, padding:10, marginBottom:8 }}>
      ...
      <button onClick={handleSave} disabled={!name.trim()}
        style={{ width:'100%', padding:'11px', background:'#C2410C', color:'#fff', border:'none', borderRadius:8, fontSize:14, fontWeight:700, cursor:!name.trim()?'not-allowed':'pointer', opacity:!name.trim()?0.5:1 }}>
        저장
      </button>
    </div>
  )
...
function PresetCard({ preset, onEdit, onDelete, onInsert, compact }) {
  const catColor = { '내과일반':'#C2410C','상기도감염':'#2563eb','소화기':'#059669','근골격/통증':'#7c3aed','소아과':'#db2777','피부':'#d97706','이비인후과':'#0891b2','외상':'#dc2626','비뇨기':'#6366f1','기타':'#6b7280' }
  const c = catColor[preset.category] || '#6b7280'
  return (
    <div style={{ background:'#fff', borderRadius:11, padding:'13px 14px', border:'1px solid #F3EFE7', borderLeft:`3px solid ${c}` }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:8 }}>
        <div>
          <div style={{ display:'flex', alignItems:'center', gap:7, marginBottom:3 }}>
            <span style={{ fontSize:14, fontWeight:700, color:'#1C1917' }}>{preset.name}</span>
            {preset.shortcut && (
              <span style={{ fontSize:11, background:'#FEF7F0', color:'#C2410C', border:'1px solid #FED7AA', borderRadius:5, padding:'1px 7px', fontWeight:700, fontFamily:'monospace' }}>#{preset.shortcut}</span>
            )}
          </div>
        ...
        <div style={{ display:'flex', gap:6, flexShrink:0 }}>
          {onInsert && <button onClick={onInsert} style={{ background:'#C2410C', color:'#fff', border:'none', borderRadius:6, padding:'5px 12px', fontSize:12, fontWeight:700, cursor:'pointer' }}>삽입</button>}
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
        {(preset.drugs||[]).map((d, i) => (
          <div key={i} style={{ display:'flex', gap:6, alignItems:'center', fontSize:12 }}>
            <span style={{ color:'#1C1917', fontWeight:500, flex:1 }}>💊 {d.name}</span>
    ...
    <div style={{ paddingBottom: 32 }}>
      ...
      <div style={{ padding:'12px 16px 10px', display:'flex', gap:10 }}>
        ...
        <button onClick={() => { setSheet('add'); setEditTarget(null) }}
          style={{ background:'#C2410C', color:'#fff', border:'none', borderRadius:8, padding:'8px 16px', fontSize:13, fontWeight:700, cursor:'pointer', whiteSpace:'nowrap' }}>
          + 추가
        </button>
      </div>
          ...
          {CATEGORIES.map(c => {
            ...
            return (
              <button key={c} onClick={() => setCatFilter(c)}
                style={{ padding:'5px 12px', borderRadius:20, border: catFilter===c?'none':'1px solid #e5e7eb', background: catFilter===c?'#C2410C':'#fff', color: catFilter===c?'#fff':'#6b7280', fontSize:12, cursor:'pointer', whiteSpace:'nowrap', fontWeight: catFilter===c?700:400, flexShrink:0 }}>
      ...
      <div style={{ padding:'0 16px', display:'flex', flexDirection:'column', gap:10 }}>
        {filtered.length === 0
          ? <div style={{ textAlign:'center', padding:'48px 0', color:'#9ca3af' }}>
              ...
              <button onClick={() => setSheet('add')} style={{ background:'#C2410C', color:'#fff', border:'none', borderRadius:20, padding:'7px 18px', fontSize:13, fontWeight:700, cursor:'pointer' }}>첫 약속처방 추가하기</button>
            </div>
          : filtered.map(p => (
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', gap:5, overflowX:'auto', marginBottom:12, paddingBottom:2 }}>
        {CATEGORIES.map(c => (
          <button key={c} onClick={() => setCatFilter(c)}
            style={{ padding:'4px 10px', borderRadius:20, border: catFilter===c?'none':'1px solid #e5e7eb', background: catFilter===c?'#C2410C':'#fff', color: catFilter===c?'#fff':'#6b7280', fontSize:11, cursor:'pointer', whiteSpace:'nowrap', flexShrink:0 }}>
            {c}
          </button>
        ))}
      </div>
            ...
            {filtered.map(p => {
              ...
              return (
                <div key={p.id} onClick={() => { onInsert(p); onClose() }}
                  style={{ background:'#FAF7F1', borderRadius:9, padding:'11px 13px', cursor:'pointer', borderLeft:`3px solid ${c}`, transition:'background 0.1s' }}
                  onMouseEnter={e => e.currentTarget.style.background='#FEF7F0'} onMouseLeave={e => e.currentTarget.style.background='#FAF7F1'}>
                  <div style={{ display:'flex', alignItems:'center', gap:7, marginBottom:5 }}>
                    <span style={{ fontSize:13, fontWeight:700, color:'#1C1917' }}>{p.name}</span>
                    {p.shortcut && <span style={{ fontSize:10, background:'#FEF7F0', color:'#C2410C', border:'1px solid #FED7AA', borderRadius:4, padding:'1px 6px', fontWeight:700, fontFamily:'monospace' }}>#{p.shortcut}</span>}
