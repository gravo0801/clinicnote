...
const S = {
  row: { display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', background: '#FAF7F1', borderRadius: 8, border: '1px solid #F3EFE7', marginBottom: 8 },
  ...
  btn: function(active) { return { fontSize: 11, color: active ? '#C2410C' : '#6b7280', background: active ? '#FEF7F0' : '#fff', border: '1px solid #e5e7eb', borderRadius: 5, padding: '2px 8px', cursor: 'pointer', fontWeight: 600 } },
  ...
  input: { width: '100%', padding: '9px 11px', borderRadius: 8, border: '1px solid #e5e7eb', fontSize: 13, outline: 'none', boxSizing: 'border-box', fontFamily: 'inherit', background: '#fff', color: '#1C1917' },
  ...
  return (
    <div style={{ marginBottom: 8 }}>
      <div style={S.row}>
        ...
        <span style={{ fontSize: 12, fontWeight: 600, color: '#1C1917', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{file.name}</span>
    ...
    <div style={{ marginBottom: 8 }}>
      <div style={S.row}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: '#1C1917', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{link.title || link.url}</div>
    ...
    <div>
      <div style={{ display: 'flex', gap: 6, marginBottom: 6 }}>
        ...
        <button onClick={add} disabled={!url.trim()}
          style={{ padding: '8px 14px', background: url.trim() ? '#C2410C' : '#d1d5db', color: '#fff', border: 'none', borderRadius: 7, fontSize: 12, fontWeight: 700, cursor: url.trim() ? 'pointer' : 'not-allowed', flexShrink: 0 }}>추가</button>
      </div>
      ...
      {links.length > 0 && (
        <div style={{ marginTop: 10 }}>
          {links.map((l, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '6px 10px', background: '#FAF7F1', borderRadius: 7, marginBottom: 5, border: '1px solid #F3EFE7' }}>
...
function AiSearch({ title, content, category }) {
  ...
  return (
    <div style={{ background: '#FEF7F0', borderRadius: 10, padding: '14px', border: '1px solid #FDBA74', marginTop: 14 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: result || error ? 14 : 0 }}>
        <div>
          <div style={{ fontSize: 13, fontWeight: 700, color: '#C2410C' }}>AI 완성 처방 세트</div>
          <div style={{ fontSize: 11, color: '#6b7280', marginTop: 2 }}>상병코드 + 주처방 + 대증치료 + 위장보호 포함</div>
        </div>
        <button onClick={search} disabled={loading}
          style={{ padding: '8px 16px', borderRadius: 7, border: 'none', background: loading ? '#d1d5db' : '#C2410C', color: '#fff', fontSize: 12, fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', flexShrink: 0 }}>
        ...
        <div>
          {result.kcdCodes && result.kcdCodes.length > 0 && (
            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: '#C2410C', marginBottom: 6 }}>KCD 상병코드</div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                {result.kcdCodes.map((k, i) => <span key={i} style={{ fontSize: 12, background: '#e6f4ef', color: '#C2410C', borderRadius: 6, padding: '3px 10px', fontWeight: 700, border: '1px solid #FDBA74' }}>{k.code}  {k.name}</span>)}
              </div>
            </div>
          )}
          {result.regimen && result.regimen.length > 0 && (
            <div style={{ marginBottom: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: '#1C1917' }}>{result.prescriptionTitle || '완성 처방 세트'}</div>
                <span style={{ fontSize: 10, background: '#C2410C', color: '#fff', borderRadius: 10, padding: '1px 8px', fontWeight: 700 }}>{'전체 ' + result.regimen.length + '종 함께 처방'}</span>
              </div>
              <div style={{ background: '#fff', borderRadius: 10, border: '1px solid #e5e7eb', overflow: 'hidden' }}>
                <div style={{ background: '#FAF7F1', padding: '7px 12px', fontSize: 11, color: '#6b7280', borderBottom: '1px solid #e5e7eb' }}>아래 약물 전체가 하나의 완성된 처방 세트입니다.</div>
                {result.regimen.map((r, i) => {
                  const rs = getRoleStyle(r.role)
                  return (
                    <div key={i} style={{ padding: '10px 14px', borderBottom: i < result.regimen.length - 1 ? '1px solid #F3EFE7' : 'none', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                      <div style={{ width: 3, flexShrink: 0, alignSelf: 'stretch', background: rs.border, borderRadius: 2, marginTop: 2 }} />
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 4, flexWrap: 'wrap' }}>
                          <span style={{ fontSize: 13, fontWeight: 700, color: '#1C1917' }}>{r.drug}</span>
                          ...
                          <span style={{ fontSize: 11, color: r.covered !== false ? '#C2410C' : '#dc2626', background: r.covered !== false ? '#FEF7F0' : '#fee2e2', borderRadius: 4, padding: '1px 6px', fontWeight: 600, marginLeft: 'auto', flexShrink: 0 }}>{r.covered !== false ? '급여' : '비급여'}</span>
                        </div>
  ...
  return (
    <div style={{ paddingBottom: 20 }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
          {CATEGORIES.filter(c => c !== '전체').map(c => (
            <button key={c} onClick={() => setCat(c)}
              style={{ padding: '4px 10px', borderRadius: 20, border: category === c ? 'none' : '1px solid #e5e7eb', background: category === c ? '#C2410C' : '#fff', color: category === c ? '#fff' : '#6b7280', fontSize: 12, cursor: 'pointer', fontWeight: category === c ? 700 : 400 }}>
              {c}
            </button>
          ))}
        </div>
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <label style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '7px 13px', background: '#FEF7F0', color: '#C2410C', border: '1px dashed #FB923C', borderRadius: 8, fontSize: 12, cursor: 'pointer', fontWeight: 600 }}>
      ...
      <div style={{ marginBottom: 16, background: '#FAF7F1', borderRadius: 10, padding: '12px 14px', border: '1px solid #F3EFE7' }}>
      ...
      <button onClick={handleSave} disabled={disabled}
        style={{ width: '100%', padding: '12px', background: disabled ? '#d1d5db' : '#C2410C', color: '#fff', border: 'none', borderRadius: 9, fontSize: 14, fontWeight: 700, cursor: disabled ? 'not-allowed' : 'pointer' }}>
...
function NoteCard({ note, onEdit, onDelete }) {
  ...
  return (
    <div style={{ border: `1px solid ${open ? '#D6D0C2' : '#E7E2D7'}`, borderRadius: 14, overflow: 'hidden', marginBottom: 10, boxShadow: open ? '0 4px 20px rgba(0,0,0,0.07), 0 1px 4px rgba(0,0,0,0.04)' : '0 1px 3px rgba(0,0,0,0.04)' }}>
      <div onClick={() => setOpen(p => !p)} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '13px 16px', background: open ? '#FEF7F0' : '#fff', cursor: 'pointer', userSelect: 'none' }}>
        <div style={{ width: 34, height: 34, borderRadius: 9, background: '#C2410C', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, flexShrink: 0, marginTop: 1, color: '#fff', fontWeight: 700 }}>N</div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 14, fontWeight: 600, color: '#1C1917', marginBottom: 5, letterSpacing: '-0.2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{note.title || '제목 없음'}</div>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
            <span style={{ fontSize: 11, color: '#9ca3af' }}>{dateStr}</span>
            {note.category && <span style={{ fontSize: 10, color: '#C2410C', background: '#FEF7F0', border: '1px solid #FED7AA', borderRadius: 10, padding: '1px 7px', fontWeight: 600 }}>{note.category}</span>}
            ...
            {(note.tags || []).map(t => <span key={t} style={{ fontSize: 10, color: '#C2410C', background: '#FEF7F0', border: '1px solid #FED7AA', borderRadius: 10, padding: '1px 6px' }}>{t}</span>)}
          </div>
        </div>
      ...
      {open && (
        <div style={{ padding: '16px', borderTop: '1px solid #F3EFE7', background: '#fff' }}>
          {hasText && <div style={{ fontSize: 13, color: '#1C1917', lineHeight: 1.85, whiteSpace: 'pre-wrap', marginBottom: 14, background: '#FAF7F1', borderRadius: 8, padding: '12px 14px', border: '1px solid #F3EFE7' }}>{note.content}</div>}
          {imgCount > 0 && (
            <div style={{ marginBottom: 14 }}>
...
function Pagination({ total, page, perPage, onChange }) {
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 6, paddingTop: 20, paddingBottom: 12 }}>
      ...
      {pages.map(p => <button key={p} onClick={() => onChange(p)} style={{ padding: '6px 11px', borderRadius: 7, border: page === p ? 'none' : '1px solid #e5e7eb', background: page === p ? '#C2410C' : '#fff', color: page === p ? '#fff' : '#374151', cursor: 'pointer', fontSize: 13, fontWeight: page === p ? 700 : 400 }}>{p}</button>)}
...
function EmptyState({ onAdd, search }) {
  return (
    <div style={{ textAlign: 'center', padding: '60px 20px', color: '#9ca3af' }}>
      ...
      {!search && <button onClick={onAdd} style={{ background: '#C2410C', color: '#fff', border: 'none', borderRadius: 20, padding: '10px 24px', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>첫 노트 작성하기</button>}
    </div>
  )
}
...
export default function DiseaseNoteTab() {
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 8000, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', overflowY: 'auto', padding: '16px 12px' }}>
      <div style={{ background: '#fff', borderRadius: 14, width: '100%', maxWidth: 960, padding: '28px 36px', boxShadow: '0 20px 60px rgba(0,0,0,0.2)', marginTop: 16, marginBottom: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, paddingBottom: 14, borderBottom: '1px solid #F3EFE7' }}>
          <div style={{ fontSize: 17, fontWeight: 700, color: '#1C1917' }}>{editTarget ? '노트 수정' : '새 질환 노트'}</div>
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      <div style={{ width: 240, background: '#fff', borderRight: '1px solid #E7E2D7', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
        <div style={{ padding: '14px 12px 10px', borderBottom: '1px solid #F3EFE7' }}>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="노트 검색..." style={{ ...S.input, fontSize: 12, padding: '7px 9px', background: '#F9F6F1', border: '1.5px solid #E7E2D7' }} />
        </div>
        <div style={{ flex: 1, overflowY: 'auto', padding: '8px' }}>
          {CATEGORIES.map(c => <button key={c} onClick={() => setCatFilter(c)} style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '7px 10px', borderRadius: 8, border: 'none', background: catFilter === c ? '#FEF7F0' : 'transparent', color: catFilter === c ? '#C2410C' : '#374151', fontSize: 13, fontWeight: catFilter === c ? 700 : 400, cursor: 'pointer', marginBottom: 2, textAlign: 'left' }}>
            <span>{c}</span>
            {catCounts[c] > 0 && <span style={{ fontSize: 11, background: catFilter === c ? '#FFEDD5' : '#f3f4f6', color: catFilter === c ? '#C2410C' : '#9ca3af', borderRadius: 10, padding: '1px 7px' }}>{catCounts[c]}</span>}
          </button>)}
        </div>
        <div style={{ padding: '12px', borderTop: '1px solid #F3EFE7' }}>
          <button onClick={() => setShowForm(true)} style={{ width: '100%', padding: '11px', background: '#C2410C', color: '#fff', border: 'none', borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: 'pointer', boxShadow: '0 4px 14px rgba(194,65,12,0.3)', letterSpacing: '-0.2px' }}>+ 새 질환 노트</button>
        </div>
      </div>
      <div style={{ flex: 1, overflowY: 'auto', background: '#F9F6F1', padding: '24px 28px 40px' }}>
