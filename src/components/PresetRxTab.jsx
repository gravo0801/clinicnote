...
function DrugNameInput({ value, onChange, suggestions }) {
  ...
  const hits = value.length >= 1 ? suggestions.filter(s => s.toLowerCase().includes(value.toLowerCase())).slice(0, 7) : []
  useEffect(() => {
  ...
  return (
    <div ref={ref} style={{ position:'relative' }}>
      ...
      {open && hits.length > 0 && (
        <div style={{ position:'absolute', top:'100%', left:0, right:0, zIndex:400, background:'#fff', border:'1px solid #d1fae5', borderRadius:6, boxShadow:'0 6px 20px rgba(0,0,0,0.12)', overflow:'hidden' }}>
          {hits.map(n => (
